...
class PIS_Requests_logic{
      
  function save_PIS_Requests($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Editing PIS_Requests");
...
//   Function that runs after saving the data in the database
  function PIS_Requests_saved($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Saved PIS_Requests");

// // 	Reference values stored in the measurement
//     require_once('PIS_Requestss/PREF_MODUL/PREF_MODUL.php');
//     $PREF_MODUL = new PREF_MODUL();
//     $PREF_MODU = $PREF_MODUL->retrieve($bean->ids_global_id);
    
//     $codigo = $PREF_MODU->number;
//     $query = "UPDATE PIS_Requests SET identifier = CONCAT('{$codigo}', '-',LPAD(number, 2, '0')) WHERE id = '{$bean->id}' ";
// // 	$log->fatal($query);
//     $bean->db->query($query);
