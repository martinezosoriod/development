...
class PIS_CSuppliers_logic{
      
  function save_PIS_CSuppliers($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Editing PIS_CSuppliers");
...
//   Function that runs after saving the data in the database
  function PIS_CSuppliers_saved($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Saved PIS_CSuppliers");

// // 	Reference values stored in the measurement
//     require_once('PIS_CSupplierss/PREF_MODUL/PREF_MODUL.php');
//     $PREF_MODUL = new PREF_MODUL();
//     $PREF_MODU = $PREF_MODUL->retrieve($bean->ids_global_id);
    
//     $codigo = $PREF_MODU->number;
//     $query = "UPDATE PIS_CSuppliers SET identifier = CONCAT('{$codigo}', '-',LPAD(number, 2, '0')) WHERE id = '{$bean->id}' ";
// // 	$log->fatal($query);
//     $bean->db->query($query);

// // 	Update dependencies
//     $query =
//     "UPDATE PIS_CSuppliers ".
