...
class Irw_myreviews_logic{
      
  function save_Irw_myreviews($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Editing Irw_myreviews");
...
//   Function that runs after saving the data in the database
  function Irw_myreviews_saved($bean, $event, $arguments){

    global $app_list_strings;

    $log = $GLOBALS['log'];
    $log->fatal("Saved Irw_myreviews");

// // 	Reference values stored in the measurement
//     require_once('Irw_myreviewss/PREF_MODUL/PREF_MODUL.php');
//     $PREF_MODUL = new PREF_MODUL();
//     $PREF_MODU = $PREF_MODUL->retrieve($bean->ids_global_id);
    
//     $codigo = $PREF_MODU->number;
//     $query = "UPDATE Irw_myreviews SET identifier = CONCAT('{$codigo}', '-',LPAD(number, 2, '0')) WHERE id = '{$bean->id}' ";
// // 	$log->fatal($query);
//     $bean->db->query($query);

// // 	Update dependencies
//     $query =
//     "UPDATE Irw_myreviews ".
